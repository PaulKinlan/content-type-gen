...
import { type PageData, type PageEnricher, passthroughEnricher } from "./generate.ts";
  ...
  return async (data: PageData, probe: MediaProbe): Promise<PageData> => {
    ...
    try {
      const prompt =
        ...
        `filename, duration ${Math.round(probe.durationSec)}s, kind ${probe.kind}. ` +
      ...
      return {
        ...data,
        title: typeof out.title === "string" && out.title ? out.title : data.title,
        description: typeof out.description === "string" && out.description ? out.description : data.description,
